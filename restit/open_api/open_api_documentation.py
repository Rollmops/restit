...
class OpenApiDocumentation:
    ...
    def register_resource(self, resource: Resource):
        ...
        """
        if (
            resource not in self._resources
            and resource.__class__.__name__ not in OpenApiDocumentation._IGNORE_RESOURCE_CLASS_NAMES
        ):
    ...
    def _add_resource(self, paths: dict, resource: Resource, root_spec: dict):
        ...
        paths[path] = {"summary": summary, "description": description}
        for method_name, method_object in self._get_allowed_resource_methods(resource).items():
            paths[path][method_name] = {"responses": {}, "parameters": []}
    ...
    def _add_request_body(method_spec: dict, method_object: object, root_spec: dict):
        request_body_parameter = getattr(
            method_object, "__request_body_properties__", None
    ...
    def _add_path_parameters(
        method_spec: dict,
        resource: Resource,
        inferred_path_parameters: List[PathParameter],
    ):
        parameter_definitions = getattr(resource, "__path_parameters__", [])
        path_parameters = {path_parameter.name: path_parameter for path_parameter in inferred_path_parameters}
        path_parameters.update({path_parameter.name: path_parameter for path_parameter in parameter_definitions})
    ...
    @staticmethod
    def _infer_path_params_and_open_api_path_syntax(
        path: str,
    ...
    @staticmethod
    def _get_summary_and_description_from_doc(
        doc: str,
    ...
    def _get_allowed_resource_methods(resource: Resource) -> dict:
        allowed_methods_dict = {
            allowed: getattr(resource, allowed)
            for allowed in resource.get_allowed_methods()
